...
resource "aws_ecs_service" "ecs_connector_service" {
  name            = "${var.connector_app_name}-ecs-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  task_definition = aws_ecs_task_definition.ecs_connector_task_definition.arn
  desired_count   = var.connector_app_count
  launch_type     = "EC2"
  ...
  load_balancer {
    ...
    container_name   = "${var.connector_app_name}-container"
    container_port   = var.connector_server_port
  }
...
resource "aws_ecs_task_definition" "ecs_mm_connector_task_definition" {
  family             = "${var.mm_connector_app_name}-task-definition"
  execution_role_arn = aws_iam_role.ecs_task_execution_role.arn
  network_mode       = "bridge"
  requires_compatibilities = [
    "EC2",
  ]
  cpu    = var.mm_connector_container_cpu
  memory = var.mm_connector_container_memory

  container_definitions = templatefile(
    "${path.module}/templates/task_definition_mm_connector.json.tpl",
    {
      mm_connector_app_name    = var.mm_connector_app_name
      mm_connector_ecr_repo    = var.mm_connector_ecr_repo
      mm_connector_server_port = var.mm_connector_server_port
      aws_region               = var.region
      mm_container_version     = var.mm_container_version
    }
  )
...
// Means and Merits Connector Resource
resource "aws_ecs_service" "ecs_mm_connector_service" {
  name            = "${var.mm_connector_app_name}-ecs-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  task_definition = aws_ecs_task_definition.ecs_mm_connector_task_definition.arn
  desired_count   = var.mm_connector_app_count
  launch_type     = "EC2"
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.mm_connector_target_group.id
    container_name   = "${var.mm_connector_app_name}-container"
    container_port   = var.mm_connector_server_port
  }

  depends_on = [
    aws_lb_listener.mm_connector_listener,
    aws_iam_role_policy_attachment.ecs_task_execution_role,
  ]
