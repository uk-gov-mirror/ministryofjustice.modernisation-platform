...
data "aws_vpc" "shared" {
  tags= {
    "Name" = "${var.networking[0].business-unit}-${var.environment}"
  }
}
data "aws_subnet_ids" "shared-private" {
  vpc_id = data.aws_vpc.shared.id
  tags = {
    "Name" = "${var.networking[0].business-unit}-${var.environment}-${var.networking[0].set}-private*"
  }
}
...
resource "aws_ecs_task_definition" "ecs_task_definition" {
  ...
  container_definitions = templatefile(
    "${path.module}/templates/task_definition_opahub.json.tpl",
    {
      ...
      db_host           = data.aws_db_instance.database.endpoint
      as_hostname       = var.endpoint
      db_user           = "opahubdb"
      db_password       = "2jAP6q6pd0lc"
      hub_password      = "WVwzkB7E4Q4BU!"
...
resource "aws_security_group" "efs-security-group" {
  name_prefix = "${var.app_name}-efs-security-group"
...
resource "aws_cloudwatch_metric_alarm" "Ecs_Memory_Over_Threshold" {
  ...
  alarm_description   = "${var.account_name} | OPA Hub ECS average memory usage is above 75% please investigate, runbook - https://dsdmoj.atlassian.net/wiki/spaces/CCMS/pages/1408598133/Monitoring+and+Alerts"
...
resource "aws_cloudwatch_metric_alarm" "EC2_CPU_over_Threshold" {
  ...
  alarm_description   = "${var.account_name} | OPA Cluster EC2 CPU utilisation is above 85% please investigate, runbook - https://dsdmoj.atlassian.net/wiki/spaces/CCMS/pages/1408598133/Monitoring+and+Alerts"
  comparison_operator = "GreaterThanThreshold"
  metric_name         = "CPUUtilization"
  statistic           = "Average"
  namespace           = "AWS/EC2"
...
resource "aws_cloudwatch_metric_alarm" "Status_Check_Failure" {
  ...
  alarm_description   = "${var.account_name} | An OPA HUB cluster EC2 instance has failed a status check, please investigate, runbook - https://dsdmoj.atlassian.net/wiki/spaces/CCMS/pages/1408598133/Monitoring+and+Alerts"
  comparison_operator = "GreaterThanThreshold"
